...
for n = 1:numPts
    ...
    % Initialize vectors to store the Euclidean distance differences
    distance_euclidean = zeros(1, k);
    % Initialize vectors to store the Gaussian kernel weights based on distance differences
    gausskernel_euclidean = zeros(1, k);

    % Iterate through each neighbor of the current point and cluster the neighborhood according to direction and distance differences
    for i = 1:k
        ...
        % Calculate the Gaussian kernel weight based on Euclidean distance differences
    ...
    while error > tol
        ...
        for i = 1:k
            ...
            newdistance(i) = norm((oldx - plo.Location(i, :)) / sigma).^2;
        ...
        % x_d = x + hunhe_d(n).*dot(np,(x_m - x))*np; % New point
